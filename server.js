...
function mergeStreamData(parts) {
  ...
  return dedupeMergedStreamData(merged);
}

function dedupeMovieStreams(movieStreams) {
  if (!movieStreams || typeof movieStreams !== 'object' || Array.isArray(movieStreams)) {
    return {};
  }

  const deduped = {};

  Object.entries(movieStreams).forEach(([imdbId, streams]) => {
    if (!Array.isArray(streams)) {
      return;
    }
...
function dedupeAuthorizedIndex(authorizedIndex) {
  if (!Array.isArray(authorizedIndex)) {
    return [];
  }

  const byImdb = new Set();
  const byTitleYear = new Set();
  const deduped = [];

  authorizedIndex.forEach((entry) => {
    if (!entry || typeof entry !== 'object') {
      return;
    }

    const imdbId = typeof entry.imdbId === 'string' ? entry.imdbId.trim() : '';
    const year = Number.isInteger(entry.year) ? entry.year : undefined;
...
function dedupeMergedStreamData(data) {
  return {
    movieStreams: dedupeMovieStreams(data.movieStreams),
    authorizedIndex: dedupeAuthorizedIndex(data.authorizedIndex)
  };
}
